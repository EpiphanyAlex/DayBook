...
> **状态作用域**：`review` 指 M0 链路验证的当前实现切片；第一次正式结论仍为 `no_go`，独立新样本复测未完成。本文另登记 M1 审核界面的运行时改进与 M4 可插拔与打包的后端边界，均未实施；开工顺序仍受 [`docs/PRD.md` §9.4](../PRD.md) 约束。
...
| R9（**新增 2026-09-05**） | M1 实时事件的可见延迟、原始输出与待发布队列上限（含超长单行）、取消/超时后的读取与日志收尾时限；任务事件/权威快照的衔接与重复/乱序识别；无法完成收尾或持久化时的错误契约 | 本文 §3.4/§6.2；[00 地基 §3.7](./00-foundation.md) 错误契约；[03 审核 §3.8](./03-review.md) 状态投影 | **M1 开工前由维护者审定边界方案**，用零额度假 CLI 与可控存储失败验证容量和时间参数后回写；需要新增错误码或 IPC 契约时先同步 00/03。当前不照搬其他产品的超时或行数，不视为已经实现 |
...
本组不属于 M0 门禁，不要求调用真实 CLI。§5 R9 的参数与错误契约须在 M1 开工前回写；实施时将以下场景落实为使用假 CLI、临时数据目录的自动测试，并将真实测试命令补回本节：
