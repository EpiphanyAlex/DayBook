...
> **状态作用域**：`review` 指 M0 链路验证的当前实现切片；第一次正式结论仍为 `no_go`，独立新样本复测未完成。维护者于 2026-09-06 开放的[有限 M1 并行切片](../PRD.md)只含 03 审核的 design token、Query + reducer 与完整原件证据，不含本文的实时事件、有界输出或收尾；本文登记的 M1 运行时改进与 M4 后端边界仍未实施。
...
#### M1：实时事件与有界收尾（2026-09-05 决定，未实施）

依据本节日志分级、[ADR-0007](../adr/0007-local-observability-and-log-tiers.md) 的本机排障要求，以及 [§7](#7-回流记录) 2026-08-23「解析中看不到日志」的实测缺口，M1 优先补齐以下边界：

- **事件按来源使用**：CLI 的结构化事件提供运行阶段、耗时与用量；已生成草稿数取成功持久化的 MCP 写入。模型自由文本不得直接成为业务进度或成功结论；最终结果仍由本节完成协议、attempt 与来源状态决定。用量暂不可得时保留未知；各后端须区分增量与累计用量，避免覆盖漏计或重复相加。
- **执行期间可观测**：持续排空 stdout 与 stderr，运行事件在会话结束前即可用于本机排障。内存中的原始输出缓冲必须有上限，包含超长单行；需要保留的完整 debug 流与工具调用参数增量落盘，继续遵守本节开关、内容分级与保留期。缓冲截断不能悄悄截掉必需的协议事件或把未知结果变成成功。
- **收尾必须有界**：取消或硬超时后，进程组回收、输出读取任务结束与日志 flush 均须有明确的等待上限；CLI 主进程退出但后代仍持有管道时也不能无限等待 EOF。失败仍按 attempt 作废草稿并保留审计；不得把未回收的进程或未完成的收尾报告为成功取消。
- **实现前先确定参数**：事件可见延迟、缓冲上限、输出读取与日志收尾时限、收尾异常的错误契约见 §5 R9。零额度假 CLI 必须能使用短超时验证这些分支，不能依赖等待真实任务硬超时；验收见 §6.2。

**pi 设计回流（2026-09-05，M1 未实施）**：参考固定版本 [`earendil-works/pi` `9841914`（包版本 0.85.0）](https://github.com/earendil-works/pi/tree/9841914c71a74d81abe07f751aefd271fd924e63)。依据是源码与测试阅读，未运行真实模型；其 CLI 使用的 `Agent` 循环与另行实现的持久化 `AgentHarness` 须分别看待，不能把后者能力视为 CLI 默认行为。借鉴的是下列边界，不是引入该依赖：

- **接收、进度、解析结果与入账分别取证**：命令接收或排队成功仅表示请求已接受；起草进度以本次 attempt 已持久化草稿为准；解析完成须通过本节完成协议；入账只认 [03 审核与草稿区 §3.1](./03-review.md) 的人工确认事务。CLI 终结事件、进度摘要或「已完成」文本均不能越过这些边界。参考 [pi RPC 接收语义](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/coding-agent/docs/rpc.md) 与 [持久化结果设计](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/docs/tool-durability.md)。
- **事件绑定任务身份，终态封闭写入**：来源任务事件携带 `source_id`、`attempt_id` 与 `agent_session_id`；无 attempt 的 readiness probe 事件独立标识，不能推进来源任务。消费者按任务身份隔离，重复或过期进度不得重复计数、回退终态或覆盖新 attempt；来源切换、重试与取消后同样适用。Rust 对进入终态或已关闭会话的迟到工具写入/回调须在写入边界拒绝，不能只靠 UI 丢弃。历史日志可按原任务归档，不因此恢复执行权。参考 [pi 事件投影](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/src/harness/runtime/reducer.ts) 与 [进度写入封闭](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/src/harness/runtime/progress.ts)。
- **界面观察不掌握执行权**：UI 订阅失败、面板关闭或组件卸载不能改变持久化业务结果，也不能隐式取消/重试 agent。重新订阅、检测到事件缺口或身份不符时，从 Rust 重取权威快照；事件只驱动显示或定向失效，不在前端重建来源状态机、确认策略或对账结果。快照与事件如何衔接、如何识别重复/乱序列入 §5 R9；界面边界同步 [03 §3.8](./03-review.md)。日志持久化失败仍须按 R9 错误契约报告，不能与纯 UI 消费失败混为一谈。参考 [pi 被动事件总线](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/src/harness/events.ts)。
- **持续读取与界面发布分开**：可合并高频进度、按最新快照节流发布，不能因此暂停管道排空、丢弃完成/失败判定所需的协议信息或漏记必须持久化的完整工具调用。展示截断须明确标记，并保留原任务身份；字符数、行数与字节数不能混用，覆盖 Unicode 分块与超长单行。内存容量、待发布队列上限、发布延迟与收尾时限由 R9 决定，不照搬 pi 的数值。参考 [pi 有界输出与发布实现](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/src/harness/utils/output-capture.ts)。
- **未知结果保留为未知**：日志或进度 checkpoint 不能证明模型/工具副作用已经完成；从已提交的 attempt、草稿与审计恢复事实，不根据最后一条进度猜成功。M1 保持 [02 导入 §3.4/§3.5](./02-ingest.md) 的启动中断收束与用户显式重试，不引入 pi 的自动重放、会话树、多 lane 或持久化执行平台。原件、模型上下文与 UI 投影仍分离，不用摘要取代原件或把对话当作 [06 记忆](./06-memory.md) 的规则。

**验证方法**：在 §6.2 现有假 CLI 场景上增加可控存储失败、重复/迟到事件和界面重新订阅，检查提交前后与会话关闭边界；只有事务提交的事实可成为恢复依据。借鉴 [pi 的可控存储与假模型验收](https://github.com/earendil-works/pi/blob/9841914c71a74d81abe07f751aefd271fd924e63/packages/agent/test/harness/runtime/accept.test.ts)，不把外部仓库的测试通过情况当作 Daybook 的验收结果。M1 不接入 pi 后端、不放开工具面，也不内嵌直连模型 SDK；第二后端仍按 §3.5 单独实施和探测。
...
#### 第二后端接入时的公共执行边界（2026-09-05 决定，未实施）

依据 [ADR-0003 §4](../adr/0003-agent-runtime-and-pluggable-backend.md) 与本节后端接口，**第二个外部进程后端接入时**统一 spawn、stdout/stderr 采集、超时、取消与进程组回收，避免每个 adapter 各自复制完整生命周期。后端差异收敛在命令与密封配置构造、安装资格与能力探测、结构化输出解释；**不得为了缩窄接口而删除 readiness 或有效能力清单验证**。来源/attempt 生命周期、完成判定与失败作废继续由 Rust 业务编排层统一负责。

当前只有 Claude Code 后端，不提前抽取公共执行器。具体 trait 签名、类型名与文件组织留给第二后端实施计划；验收要求是所有后端共用生命周期契约测试，并各自通过 §3.7 密封探测。默认归属 M4；若因 §5 R4 提前接入第二后端，公共执行边界随接入一起落实。
...
| R9（**新增 2026-09-05**） | M1 实时事件的可见延迟、原始输出与待发布队列上限（含超长单行）、取消/超时后的读取与日志收尾时限；任务事件/权威快照的衔接与重复/乱序识别；无法完成收尾或持久化时的错误契约 | 本文 §3.4/§6.2；[00 地基 §3.7](./00-foundation.md) 错误契约；[03 审核 §3.8](./03-review.md) 状态投影 | **运行事件切片开工前由维护者审定边界方案**，用零额度假 CLI 与可控存储失败验证容量和时间参数后回写；需要新增错误码或 IPC 契约时先同步 00/03。该切片不属于 2026-09-06 开放的[有限 M1 并行范围](../PRD.md)，当前不照搬其他产品的超时或行数，也不视为已经实现 |
...
### 6.2 M1 实时事件与收尾（未实施；不在有限并行切片）

本组不属于 M0 门禁，也不属于 [`docs/PRD.md` 2026-09-06 开放的有限 M1 并行切片](../PRD.md)，不得随 Query 状态迁移顺带实现。§5 R9 的参数与错误契约须在运行事件切片开工前回写；届时将以下场景落实为使用假 CLI、临时数据目录的自动测试，并将真实测试命令补回本节：

| 场景 / 操作 | 成功判据 |
|---|---|
| 假 CLI 输出阶段事件后保持运行；执行成功的 MCP 起草，再输出虚假的「全部完成」文本 | 事件在 R9 规定的延迟内可读；草稿数只反映已持久化写入；缺少完成协议时不得变成 `parsed` |
| stdout/stderr 同时持续输出，分别覆盖超长单行与大量短行 | 两个流持续被排空，原始输出缓冲不超过 R9 上限；debug 开启时完整调用仍可导出重放，关闭时 trace 不含原文、金额或 prompt |
| 在持续输出期间取消；另用短硬超时终止同类任务 | 在 R9 的收尾时限内结束；持续输出不能延长硬 deadline 或饿死取消；进程组、读取任务均回收，结果分别为取消与超时 |
| 假 CLI 主进程退出，后代继续持有 stdout/stderr；另模拟日志写入失败 | 收尾不无限等待 EOF/flush，按 R9 错误契约报告，不能伪装为正常完成或已成功取消 |
| 部分草稿写入后超时；另正常退出但不调用 `complete_source` | 前者记超时、后者记协议失败；该 attempt 草稿全部作废，原始起草值与审计保留，事实表无新增 |
| 回放后端用量事件，分别覆盖逐步增量与最终累计值 | 不漏计、不重复累计；缺失用量保持未知；用量事件不改变完成判定 |
| 请求仅被接受/排队；分别注入 CLI「完成」文本、进度末帧和真实完成协议 | 只有完成协议与持久化结果共同成立才能显示解析完成；所有解析事件均不能新增事实交易或冒充人工确认成功 |
| attempt A 结束后启动 B，再发送 A 的进度/完成事件及迟到工具写入；另重复发送 B 的同一进度 | A 不覆盖 B、不恢复写入能力；重复进度不重复计数；终态不被旧进度回退；历史日志仍归 A |
| 关闭日志面板、令 UI 消费者抛错、卸载后重新订阅，并在快照读取期间注入新事件 | 不触发隐式取消或重试；按 R9 衔接快照/事件后，展示与 Rust 当前 attempt 一致，确认策略与对账结果只来自重取 |
| 用高频 Unicode 分块输出压满待发布队列，再发送结束事件 | 原始缓冲与发布队列均受 R9 上限约束；界面可合并进度但最终结果与草稿数正确；展示截断有标识、无破损字符，debug 完整调用不因 UI 节流丢失 |
| 在草稿/attempt 事务提交前注入存储失败；另在成功提交后、UI 通知前模拟消费中断 | 提交前不得发布已持久化成功，事务不留部分业务写入；提交后从库重取实际结果，不重复执行副作用、不用进度补造成功；事实表仍仅由人工确认路径写入 |

人工验收：使用假后端驱动桌面界面，解析未结束时打开本机日志面板，按 R9 的延迟上限核对阶段事件可见；点停止后，界面与持久化 attempt 的最终结果一致。真实后端复核须另按当时授权执行。

### 6.3 第二后端接入（未实施，默认 M4）

- 公共生命周期契约测试覆盖 §6.2 的输出、取消、超时、后代持管道与失败留痕场景；新增后端不得复制一套独立的进程收尾流程。
- 每个后端分别覆盖命令构造、安装资格、认证失败、有效能力集合不等时拒绝运行，以及增量/累计用量解释。公共执行器通过测试不能替代后端自身的密封能力探测。

## 7. 回流记录

| 日期 | 回流内容 | 依据 |
|---|---|---|
| 2026-09-06（有限 M1 并行决定） | 开放范围只落在 03 审核的 design token、Query + reducer 与完整原件证据；本文的实时事件、有界输出与收尾明确排除，R9 仍须在该运行事件切片开工前另行审定。本次不改五工具、进程生命周期或 IPC，不开始实现，`review` 仍指 M0 切片 | [`docs/PRD.md`「有限 M1 并行开发边界」](../PRD.md)；[03 审核 §3.9/§6](./03-review.md) |
| 2026-09-05（pi 设计回流） | M1 补齐请求接收/进度/结果/入账的证据边界、任务身份与终态封闭、被动 UI 观察、输出节流及可控存储故障验收；R9 增加快照与事件衔接。参考固定提交的源码与测试，不把 pi 持久化 harness 当作 CLI 默认能力；不引入自动恢复或新后端，当前 M0 `review` 不变 | [本文 §3.4](#34-agent-launcher) 的 pi 固定版本与源码链接；[§6.2](#62-m1-实时事件与收尾未实施)；[03 审核 §3.8](./03-review.md) |
| 2026-09-05（harness 设计回流） | 维护者确认 M0 独立复测优先；M1 补实时事件、有界输出与有界收尾，并先用假 CLI 验收；公共进程执行逻辑待第二后端接入时统一。当前 `review` 仍只覆盖 M0 切片，未启动实现或真实复测。外部项目的经验只用于提出设计方向，未作为 Daybook 的实测结果或参数依据 | [本文 §3.4](#34-agent-launcher)、[§3.5](#35-可插拔后端接口daybook-安装资格与解析就绪度分开2026-08-17-定案)、[§6](#6-验收标准)；现有缺口见本表 2026-08-23 实测；[ADR-0003 §4](../adr/0003-agent-runtime-and-pluggable-backend.md)、[ADR-0007](../adr/0007-local-observability-and-log-tiers.md) |
...
| 版本 | 日期 | 变更 |
|---|---|---|
| v0.32 | 2026-09-06 | 明确 2026-09-06 有限 M1 并行范围不含实时事件、有界输出与收尾；R9 仍在运行事件切片开工前审定。M0 `review`、五工具、进程与正式复测边界不变，本次未实施 |
| v0.31 | 2026-09-05 | pi 源码调研决定回流到 M1：事件身份与终态封闭、观察与执行分离、有界节流和提交边界故障验收；同步 03 的 UI 投影。后续设计未实施，M0 状态与 formal 判据不变 |
| v0.30 | 2026-09-05 | 登记 M1 实时事件、有界缓冲与收尾边界、R9 待决参数及零额度验收场景；第二后端接入时统一进程生命周期。§6 按 M0/M1/第二后端分层，当前 M0 `review` 与正式 `no_go` 不变；本轮仅文档回流 |
