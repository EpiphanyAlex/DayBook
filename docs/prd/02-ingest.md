...
用户的真实动作是：过去 7–10 天里，手机上攒了一堆截图（CBA 网银、信用卡账单、微信账单、支付宝账单），现在坐下来一次性处理。
...
- 用户的判定结果进 [06 记忆](./06-memory.md)（例：「CBA 流水与信用卡账单总是重复，优先留信用卡那条」）
...
- [ ] 拖入一张真实的 CBA 网银截图，能走完 `imported → parsing → parsed`，草稿数与图上条目数一致
