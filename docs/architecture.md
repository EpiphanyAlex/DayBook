...
> **当前状态：第一次 M0 正式结果仍为 `no_go` / exit 3；修正实现与零额度门禁已完成，独立新样本正式复测尚未授权。** 维护者于 2026-09-06 只开放 [03 审核](./prd/03-review.md) 的 design token、Query + reducer 与完整原件证据有限 M1 三项；该复测不再阻止有限切片开工，但仍是 M1 整体验收门槛。结构本身仍已落地；[00 地基](./prd/00-foundation.md) v0.23、[01 Agent 运行时](./prd/01-agent-runtime.md) v0.32、[03 审核](./prd/03-review.md) v0.22、[07 评测](./prd/07-eval.md) v0.18 与 [02 导入](./prd/02-ingest.md) v0.17 当前均为 `review`；本次只回流文档，有限切片尚未实现。旧报告与旧样本永久保留，后续只用独立新样本正式复测（[`docs/PRD.md` §9.4](./PRD.md)）。
...
- **M1 状态边界**（2026-08-24 定案；2026-09-06 获准有限并行，尚未实施）：TanStack Query v5 只缓存可从 Rust/Tauri 重取的权威投影，screen reducer / 局部 state 只存选择、焦点、编辑缓冲等瞬时 UI 意图；mutation 成功后定向失效重取，不把来源状态、草稿事实或对账结果复制成前端业务 store。它与 design token、完整原件证据一起构成当前获准的有限切片，详细 query key、桌面 IPC 默认项与范围见 [03 审核 §3.8–§3.9](./prd/03-review.md)。
