...
| **其他分类** | 支出侧与收入侧各有一条名为「其他」的真实分类，表示方向已判断但没有更合适分类；在回顾中与未分类分开。两者**同名而不同实体**——名称唯一性只在同一 `scope` 内成立，脱离 scope 上下文呈现时必须标注方向 | [`04-transactions` §3.3](./prd/04-transactions.md) |
