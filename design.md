...
> **状态：已评审定稿（2026-08-24），v0.5。** 三条「待澄清」已拍定并落进正文（见「已澄清」），`ink-800` / `paper-500` 的取值口径定为 OKLCH 规整值。**本文现在是 M1 前端实现的判据**，已接进 [`.claude/rules/frontend.md`](./.claude/rules/frontend.md)。有限 M1 已在 `src/styles/tokens.css` 与 `src/styles.css` 实现本文 token 和当前主路径，已通过零额度自动与合成视觉验收；其余 M1 与后续页面仍未开放。
