...
agent 只有在 claim 唯一覆盖**当前不可变来源全部适用交易**时才报告；Rust 再按一次解析尝试的全部未作废草稿重算支出、收入或净变动，并把对账状态与批量确认策略分别返回。
...
- `report_source_total` 的范围资格是「当前不可变来源全部适用交易」，边界是导入后不变的截图 viewport / 整段口述。月度 viewport 外、其他分页、按日、分类、单笔、任意子组合计都不合格；多个候选无法用 amount/currency/kind 三元组唯一识别时也不报告。关键词只提示候选，不能触发代码完成闸门。
- M0 生产 schema 仍是 `parse_attempts.reported_total_*` 单 claim 四列。Rust 没有 OCR，不能从图片独立证明 scope；`total_check` 只在 agent 已选择单 claim 后做确定性等式。范围误报由 formal `expected.json.reconciliationScope` 和 `scopeInvalidTotalReports == 0` 检验，不能描述成生产已验证 scope。
...
- 对账 `passed` 只证明「所报告三元组与草稿和相等」，不证明 agent 选择了正确范围；scope 身份是 formal eval 的独立硬契约。
