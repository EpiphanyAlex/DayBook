...
const RULES = [
  ...
  {
    // 防的是 2026-08-24 R1 产品密封链路的真实失败：bbox 曾误指相邻行，
    // 而旧规格仍容易让实现者把「M0 推迟」顺手解释成「M1 必做」。
    id: 'm1-evidence-region-promised',
    re: /^(?![^\n]*(不在\s*M1|M1\s*(也)?不|否决|不做|不加|不可稳定))[^\n]*M1[^\n]{0,28}(才做|实现|增加|新增)[^\n]{0,24}(区域高亮|截图[^\n]{0,8}bbox)/i,
    why: 'R1 已实测关闭为「截图坐标不可稳定定位」；M1 保留完整原件 + evidence_text，不加伪精确高亮（03 §3.2/§5）',
    fix: '删除 M1 区域高亮承诺；若讨论未来重开，必须点名独立可验证坐标的新前提',
  },
  {
    // 防的是 2026-08-17 事项规格重写后最容易回流的旧定义：把事项说成
    // 「待办 + 时间日志」两端，会继续诱导实现者建立按时长汇总的时间追踪模型。
    id: 'item-time-log-endpoints',
    re: /^(?![^\n]*(不再|不是|并非|停止|删除|旧|此前))[^\n]*待办[^\n]{0,16}时间日志[^\n]{0,24}(两端|同一[^\n]{0,8}实体)/,
    why: '事项的正式模型已改为同一实体的「计划 + 结果」（总 PRD §5.2、05 §3.1）',
    fix: '改用「计划 + 结果」；解释旧定义时放进回流/变更记录或标记 legacy',
  },
  {
    // 防的是同一次规格重写证伪的真实前提：「没有系统自动痕迹」曾被推导成
    // 「不能记录实际时段」。现在允许记录用户明确陈述的分钟级结果，禁止的是自动编造。
    id: 'item-no-precise-result-time',
    re: /^(?![^\n]*(不再|不是|并非|推翻|旧|此前|禁止的是|不等于))[^\n]*(只记时长|不能记录实际时段|不记[^\n]{0,8}几点到几点|时间日志做不到精确时段)/,
    why: '用户明确口述本身就是来源；事项结果可精确到分钟，也可只是日期范围（05 §3.2）',
    fix: '改成「不从计划、点击时刻或系统活动自动编造结果；用户明确提供的实际时间可记录」',
  },
  {
    // 同一轮删除了「这周时间花哪了」：稀疏的自报用时不足以支撑可信周报。
    id: 'item-weekly-time-rollup',
    re: /^(?![^\n]*(不做|删除|非目标|不生成|不以|不提供|旧|此前))[^\n]*(这周时间花哪了|每周实际用时汇总)/,
    why: 'v1 只保留单条事项的可选结果时间，不生成每周实际用时汇总（05 §3.7）',
    fix: '回顾改为完成/未完成/重新安排/截止；旧结论放进回流/变更记录或标记 legacy',
  },
  {
    id: 'declared-total',
    re: /declared_total_/,
  ...
  },
  {
    ...
    // 只有同行明确写出 file / policy 限定才豁免；裸写「口述」不足以证明控制流正确。
    id: 'mismatch-blocks-all-batch',
    re: /^(?![^\n]*(kind\s*=\s*`?file|confirmation_policy|single_only|user_attested_batch))[^\n]*(不符|对不上|failed|校验不过|无法校验)[^\n]{0,40}((阻止|禁止|拒绝|返回错误)[^\n]{0,10}批量|批量[^\n]{0,20}(不可点|禁用|拒绝))/,
    why: '对账 `failed` 只对 `kind = file` 阻止批量；`utterance` 的准入由 `confirmation_policy` 决定（03 §3.3）',
    fix: '补 `kind = file` / `single_only` 限定，或点名 `user_attested_batch` 人工闸门',
  },
  {
