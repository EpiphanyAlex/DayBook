...
Daybook 是一个 **macOS 本地优先的桌面应用**，帮用户把「过去这段时间的钱和事」补记回来——**回溯记录器**，不是记账 app、不是待办 app。AI 的角色是**考古学家，不是输入框**。**当前状态**：M0 实现已进入 `review`，维护者 review 与真实样本 go/no-go 待完成；当前前端是功能基线，设计稿与 token design system 在 M1 开工前确定。
