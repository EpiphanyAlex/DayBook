...
1. **跑门禁**——前端 `npm run lint` · `npm run typecheck` · `npm test` · `npm run build`，Rust `cargo fmt --check` · `cargo clippy -- -D warnings` · `cargo test`，任一失败即红。改过 `docs/prd/` 还要跑 `node docs/prd/check-docs.mjs`；改过任何 `.md` 还要跑 `node scripts/check-links.mjs`；改过 `README.md` 必须在同一个提交里同步 `README.en.md`，由 `node scripts/check-readme-sync.mjs` 把关（三条文档门禁 CI 对所有 PR 强制）。
