...
- Daybook 是一个 **macOS 本地优先的桌面应用**，帮用户把「过去这段时间的钱和事」补记回来。它是**回溯记录器**，不是记账 app，也不是待办 app。
- 目标用户是「明知该记但当场记不了、只能事后补」的人；**第一优先场景是跨境双币种生活**（澳洲银行卡 + 信用卡 + 微信 + 支付宝并行）——这是国内与海外记账工具都覆盖不到的夹缝。
