...
**仓库处于第一次 M0 正式 `no_go` 后、修正实现已完成但尚未独立正式复测的阶段（2026-09-05 收口复审），不开始 M1。** Tauri/React/Rust、六表地基、五工具密封 agent 链路、截图/口述导入与审核确认已经落地；第一次 formal final 为 ignored 的 `output/m0-eval/2026-08-29T122443-349Z-first.final.json`，`verdict = no_go`、exit 3。截图池指标 1–3 全过；口述金额 `60/62` 触发硬性 no-go；声明合计可获得率 `4/20`、假警报率 `6/7`。月度 viewport 外、分页、按日、单笔 / 子组合计误报与 formal 证据 / ordinal 缺口已按 PR #27 修正：关键词降为候选、提示词限定 current-source 全覆盖，formal v2 增加完整 fixture-set 指纹、scope-invalid=0、bounded evidence、四硬字段两侧值与口述 span ordinal，纯合成 CI 回归已落地；完整 `node scripts/verify-m0.mjs --skip-live` 通过。[00 地基](./docs/prd/00-foundation.md) v0.23、[01 Agent 运行时](./docs/prd/01-agent-runtime.md) v0.31、[03 审核](./docs/prd/03-review.md) v0.21、[07 评测](./docs/prd/07-eval.md) v0.17 与 [02 导入](./docs/prd/02-ingest.md) v0.17 当前均为 `review`。第一次报告与 `fixtures/local/m0-2026-08-24` 永久保留、不修改、不重标；后续 formal 只用独立新样本。独立复测前仍只跑零额度门禁，**禁止无参数 live、`--m0-go-no-go`、`--m0-diagnose` 与任何未经再次明确授权的真实 agent 调用**。M1 已定的设计 token、证据退路与状态管理决定仍有效，但本轮不实施。版本号以各文件 frontmatter 与 [`docs/prd/INDEX.md`](./docs/prd/INDEX.md) 为准。
