...
**仓库处于第一次 M0 正式 `no_go` 后、修正实现已完成但尚未独立正式复测的阶段（2026-09-06 更新）。** 第一次 formal final 为 ignored 的 `output/m0-eval/2026-08-29T122443-349Z-first.final.json`，`verdict = no_go`、exit 3；口述金额 `60/62` 触发硬性 no-go，声明合计可获得率 `4/20`、假警报率 `6/7`。月度 viewport 外、分页、按日、单笔 / 子组合计误报与 formal 证据 / ordinal 缺口已按 PR #27 修正：关键词降为候选、提示词限定 current-source 全覆盖，formal v2 的完整 fixture-set 指纹、scope-invalid=0、bounded evidence、四硬字段两侧值、口述 span ordinal 与纯合成 CI 回归均已落地；完整 `node scripts/verify-m0.mjs --skip-live` 通过。[00 地基](./docs/prd/00-foundation.md) v0.23、[01 Agent 运行时](./docs/prd/01-agent-runtime.md) v0.32、[03 审核](./docs/prd/03-review.md) v0.24、[07 评测](./docs/prd/07-eval.md) v0.18 与 [02 导入](./docs/prd/02-ingest.md) v0.17 除 03 为有限 M1 `done` 外，其余保持 `review`。第一次报告与 `fixtures/local/m0-2026-08-24` 永久保留、不修改、不重标；后续 formal 只用独立新样本。独立复测仍未授权，**禁止无参数 live、`--m0-go-no-go`、`--m0-diagnose` 与任何未经再次明确授权的真实 agent 调用**。维护者只批准 [03 审核 §3.9](./docs/prd/03-review.md) 的有限 M1 并行开发：已定案 design token、TanStack Query + reducer 状态边界、完整原件 + `evidence_text` 安全退路；有限 M1 三项已完成零额度验收。独立复测不再是该有限切片的开工门槛，但仍是 M1 整体验收门槛；实时事件、排序、键盘流、虚拟滚动、40 笔跑测及 M2–M4 均未开放。版本号以各文件 frontmatter 与 [`docs/prd/INDEX.md`](./docs/prd/INDEX.md) 为准。
