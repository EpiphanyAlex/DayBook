...
**The first formal M0 result remains `no_go`, exit 3 (2026-08-29/30). The correction and zero-quota gates are complete, but a formal retest on an independent new sample set has not been authorized; M1 is not starting.** The Tauri / React / Rust end-to-end path remains implemented: the six-table foundation, sealed five-tool agent runtime, screenshot and utterance import, review-and-confirm, and the total cross-check. In the first formal result, screenshot metrics 1–3 all passed, but utterance amount accuracy was `60/62`, a hard no-go; reported-total availability was `4/20`, and the false-alarm rate was `6/7`. See [`docs/PRD.md` §9.4](./docs/PRD.md).

What failed was the M0 single-claim scope and formal-report evidence contract: monthly totals extending beyond the viewport, pagination totals, and per-day, single-item, or subset totals were mistaken for source-level totals. The correction now treats total words only as candidate signals, limits the prompt to current-source full coverage, and adds a complete fixture-set fingerprint, hard scope-invalid failure, bounded reconciliation evidence, both sides of the four hard fields, and utterance span ordinals to formal v2; a fully synthetic CI regression set is also in place. [00 Foundation](./docs/prd/00-foundation.md), [01 Agent runtime](./docs/prd/01-agent-runtime.md), [03 Review and drafts](./docs/prd/03-review.md), [07 Eval](./docs/prd/07-eval.md), and [02 Ingest](./docs/prd/02-ingest.md) are now all in `review`. The first reports and old local sample set remain permanent evidence and will not be edited or relabeled; a future formal retest must use an independent new sample set. **The current interface remains a functional baseline, not an approved design.** M1's settled token system, complete-original evidence fallback, and TanStack Query + reducer state boundary remain valid, but are out of scope for this correction. Status overview: [`docs/prd/INDEX.md`](./docs/prd/INDEX.md).
...
Until the independent new-sample formal retest is explicitly authorized, only the zero-quota gate is allowed:
