...
**The first formal M0 result remains `no_go`, exit 3 (2026-08-29/30). The correction and zero-quota gates are complete, but a formal retest on an independent new sample set has not been authorized.** The Tauri / React / Rust end-to-end path remains implemented: the six-table foundation, sealed five-tool agent runtime, screenshot and utterance import, review-and-confirm, and the total cross-check. In the first formal result, screenshot metrics 1–3 all passed, but utterance amount accuracy was `60/62`, a hard no-go; reported-total availability was `4/20`, and the false-alarm rate was `6/7`. See [`docs/PRD.md` §9.4](./docs/PRD.md).

What failed was the M0 single-claim scope and formal-report evidence contract: monthly totals extending beyond the viewport, pagination totals, and per-day, single-item, or subset totals were mistaken for source-level totals. The correction now treats total words only as candidate signals, limits the prompt to current-source full coverage, and adds a complete fixture-set fingerprint, hard scope-invalid failure, bounded reconciliation evidence, both sides of the four hard fields, and utterance span ordinals to formal v2; a fully synthetic CI regression set is also in place. [00 Foundation](./docs/prd/00-foundation.md), [01 Agent runtime](./docs/prd/01-agent-runtime.md), [03 Review and drafts](./docs/prd/03-review.md), [07 Eval](./docs/prd/07-eval.md), and [02 Ingest](./docs/prd/02-ingest.md) remain in `review` except 03, now `done` for only the limited M1 slice. The first reports and old local sample set remain permanent evidence and will not be edited or relabeled; a future formal retest must use an independent new sample set. **The three limited M1 frontend changes are implemented and have passed zero-quota acceptance checks; archived v9 supplies layout hierarchy for the current path only.** The maintainer has approved limited M1 work only for the settled design tokens, the complete-original + `evidence_text` safety fallback, and the TanStack Query + reducer state boundary; these three items have completed zero-quota acceptance. The independent retest is no longer the start gate for those three items, but remains the overall M1 acceptance gate; all other M1 work and M2–M4 remain closed. See [`docs/PRD.md` “有限 M1 并行开发边界”](./docs/PRD.md) and [`docs/prd/INDEX.md`](./docs/prd/INDEX.md) for the exact boundary.
...
Until the independent new-sample formal retest is explicitly authorized, the real-agent and formal paths remain closed; documentation and the limited M1 slice may use only zero-quota gates:
...
| UI token system (color ramps, type scale, spacing, component specs, the three input kinds) — **v0.5, final** | [`design.md`](./design.md) |
| Desktop reference design draft (nine screens, 1440 × 900) — **reviewed; all eight deviations folded back into the specs** | [`docs/design/README.md`](./docs/design/README.md) |
