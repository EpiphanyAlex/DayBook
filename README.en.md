...
What failed was the M0 single-claim scope and formal-report evidence contract: monthly totals extending beyond the viewport, pagination totals, and per-day, single-item, or subset totals were mistaken for source-level totals. The correction now treats total words only as candidate signals, limits the prompt to current-source full coverage, and adds a complete fixture-set fingerprint, hard scope-invalid failure, bounded reconciliation evidence, both sides of the four hard fields, and utterance span ordinals to formal v2; a fully synthetic CI regression set is also in place. [00 Foundation](./docs/prd/00-foundation.md), [01 Agent runtime](./docs/prd/01-agent-runtime.md), [03 Review and drafts](./docs/prd/03-review.md), [07 Eval](./docs/prd/07-eval.md), and [02 Ingest](./docs/prd/02-ingest.md) are now all in `review`. The first reports and old local sample set remain permanent evidence and will not be edited or relabeled; a future formal retest must use an independent new sample set. **The interface currently in code is still a functional baseline; archived v9 is the current page reference, not an implementation.** The maintainer has approved limited M1 work only for the settled design tokens, the complete-original + `evidence_text` safety fallback, and the TanStack Query + reducer state boundary; this change still contains documentation and planning only. The independent retest is no longer the start gate for those three items, but remains the overall M1 acceptance gate; all other M1 work and M2–M4 remain closed. See [`docs/PRD.md` “有限 M1 并行开发边界”](./docs/PRD.md) and [`docs/prd/INDEX.md`](./docs/prd/INDEX.md) for the exact boundary.
